...
(function () {
  ...
  class CollectionUI {
    ...
    updateCollectionButtons() {
      const available = this.cm.isAvailable();
      const hasNew = (this.cm.getState().cards.newInbox || []).length > 0;
      ...
      if (btn) {
        ...
        btn.title = available ? "Collection" : "Unlocks after Level 5";
        btn.setAttribute("aria-label", available ? "Collection" : "Unlocks after Level 5");
  ...
  class GameApp {
    ...
    updateBattlePassWidget() {
      const unlocked = this._save.battlePassUnlocked === true;
